...
import { Search, Home, Key, X, ChevronDown, Ban } from 'lucide-react';
  ...
  return (
    <section className='px-6 py-4 sm:px-6 lg:px-8'>
      <div className='mx-auto max-w-7xl'>
        {/* Single Filter Bar */}
        <div className='flex flex-wrap items-center justify-between gap-3 rounded-lg border border-purple-92 bg-white p-4'>
          {/* Left: Property Type Dropdown */}
          <PopoverPrimitive.Root open={categoryOpen} onOpenChange={setCategoryOpen}>
            <PopoverPrimitive.Trigger asChild>
              <button
                type='button'
                className='flex items-center gap-2 rounded-lg border border-purple-92 bg-white px-3 py-2 text-sm font-medium text-main-black transition-shadow hover:shadow-md shadow-[0px_0px_40px_0px_rgba(112,101,240,0.1)]'
          ...
          <div className='flex-1'>
            <div className='relative'>
              <Search
                className='absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-grey-500'
                strokeWidth={2}
              />
              <input
                type='text'
                ...
                className='w-full bg-transparent py-1.5 pl-8 pr-3 text-sm text-main-black placeholder-grey-500 outline-none'
              />
            </div>
          </div>

          {/* Right: View toggles and buttons */}
          <div className='flex items-center gap-3'>
            {/* Rent/Buy Toggle */}
            <div className='flex items-center gap-1 rounded-lg border border-grey-96 bg-grey-98 p-1'>
              <button
                ...
                className={`flex h-8 w-8 items-center justify-center rounded transition-colors ${
                  listingType === 'buy'
                    ? 'bg-main-primary text-white'
                    : 'bg-transparent text-main-black hover:bg-grey-96'
                }`}
              >
            ...
            {/* Remove All Button / Cancel & Remove Buttons */}
            {onSelect &&
              (isSelectionMode ? (
                <div className='flex items-center gap-2 flex-1 sm:flex-none'>
                  <button
                    onClick={onSelect}
                    className='flex-1 sm:flex-none flex items-center justify-center gap-2 rounded-lg border border-red-500 bg-white px-4 py-2.5 text-sm font-medium text-red-500 transition-colors hover:bg-red-50'
                  >
                  ...
                  <button
                    onClick={onCancel}
                    className='flex-1 sm:flex-none flex items-center justify-center gap-2 rounded-lg border border-grey-92 bg-white px-4 py-2.5 text-sm font-medium text-main-black transition-colors hover:bg-grey-96'
                  >
