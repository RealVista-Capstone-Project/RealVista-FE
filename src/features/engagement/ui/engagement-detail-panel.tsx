...
import { AlertCircle, X, MapPin, Home, CheckCircle2, XCircle, Clock, Trophy, Building2, User, FileText, Star } from 'lucide-react';
...
const statusStyle: Record<EngagementStatus, { bg: string; text: string; icon: React.ReactNode }> = {
  [EngagementStatus.SUBMITTED]: { bg: 'bg-amber-50', text: 'text-amber-600', icon: <Clock className='h-3.5 w-3.5' /> },
  ...
  [EngagementStatus.CANCELLED]: { bg: 'bg-muted text-muted-foreground', text: 'text-muted-foreground', icon: <XCircle className='h-3.5 w-3.5' /> },
...
export const EngagementDetailPanel = ({
  ...
  const isCurrentUserReceiver = currentUserId && engagement.receiverId === currentUserId;

  const getCounterpartyInfo = () => {
    if (engagementType === EngagementType.AGENT_PROPOSAL) {
      if (isCurrentUserInitiator) {
        return { name: engagement.receiverName || engagement.agentFullName || '', role: t('table.roleAgent') };
      } else {
    ...
    <div className='w-full lg:w-[360px] flex-shrink-0 bg-background rounded-xl shadow-sm border border-border flex flex-col max-h-[calc(100vh-120px)] sticky top-4 overflow-hidden animate-in slide-in-from-right-4 fade-in duration-300'>
          ...
          {canCancel && (
            <Dialog>
              <DialogTrigger asChild>
                <Button size='sm' className='h-8 px-3 text-xs bg-primary hover:bg-primary/90 text-primary-foreground'>
                  {t('cancel.button')}
                </Button>
              </DialogTrigger>
              <DialogContent>
      ...
      <div className='flex-1 overflow-y-auto min-h-0'>
        ...
        <div className='px-5 py-3 border-b border-border flex items-center justify-between'>
          <span className='text-xs text-muted-foreground font-medium'>{t('detail.statusLabel')}</span>
          <Badge
            variant='secondary'
        ...
        <div className='px-5 py-4 border-b border-border space-y-3'>
          <div className='flex justify-between text-sm'>
            ...
            <span className='font-medium text-foreground'>{formatDate(engagement.createdAt, 'dd/MM/yyyy HH:mm')}</span>
          </div>
          <div className='flex justify-between text-sm'>
            <span className='text-muted-foreground'>{t('detail.updatedDate')}</span>
            <span className='font-medium text-foreground'>{formatDate(engagement.updatedAt, 'dd/MM/yyyy HH:mm')}</span>
          </div>
          <div className='flex justify-between text-sm'>
          ...
          {engagementType === EngagementType.TENANT_APPLICATION && content?.title && (
            <div className='flex justify-between text-sm'>
              ...
              <span className='font-medium text-foreground text-right max-w-[180px] leading-tight'>{content.title}</span>
            </div>
          )}
        </div>
          ...
          <div className='px-5 py-4 border-b border-border space-y-4'>
            ...
            {(content.commissionRate !== undefined && content.commissionRate > 0) && (
              <div className='flex items-center gap-3 p-3 rounded-lg bg-primary/5 border border-primary/20'>
                ...
                <div>
                  <div className='text-[10px] text-muted-foreground uppercase tracking-wider'>{t('detail.commissionRate')}</div>
            ...
            {content.experienceYears !== undefined && content.experienceYears > 0 && (
              <div className='flex items-center gap-3 p-3 rounded-lg bg-primary/5 border border-primary/20'>
                ...
                <div>
                  <div className='text-[10px] text-muted-foreground uppercase tracking-wider'>{t('detail.experience')}</div>
            ...
            {content.specialty && (
              <div className='flex items-center gap-3 p-3 rounded-lg bg-primary/5 border border-primary/20'>
                ...
                <div>
                  <div className='text-[10px] text-muted-foreground uppercase tracking-wider'>{t('detail.specialty')}</div>
          ...
          <div className='px-5 py-4 border-b border-border space-y-4'>
            {content.offeredCommission && (
              <div className='flex items-center gap-3 p-3 rounded-lg bg-primary/5 border border-primary/20'>
                ...
                <div>
                  <div className='text-[10px] text-muted-foreground uppercase tracking-wider'>{t('detail.offeredCommission')}</div>
                  <div className='text-sm font-bold text-foreground'>{content.offeredCommission}</div>
                </div>
              </div>
            )}
            {content.message && (
        ...
        {engagementType === EngagementType.TENANT_APPLICATION && content && (
          <div className='px-5 py-4 border-b border-border'>
            <div className='grid grid-cols-2 gap-x-4 gap-y-4'>
              <div>
                <div className='text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5'>{t('detail.income')}</div>
                <div className='text-sm font-bold text-foreground'>
                  {content.monthlyIncome ? formatCurrency(content.monthlyIncome) + t('detail.incomePerMonth') : '—'}
                </div>
              </div>
              <div>
                <div className='text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5'>{t('detail.leaseTerm')}</div>
                <div className='text-sm font-bold text-foreground'>
                  {content.leaseTermMonths ? t('detail.leaseTermMonths', { months: content.leaseTermMonths }) : '—'}
                </div>
              </div>
              <div>
                <div className='text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5'>{t('detail.moveInDate')}</div>
                <div className='text-sm font-bold text-foreground'>
                  {content.moveInDate ? formatDate(content.moveInDate, 'dd MMM, yyyy') : '—'}
                </div>
              </div>
              <div>
                <div className='text-[10px] text-muted-foreground uppercase tracking-wider mb-0.5'>{t('detail.tenants')}</div>
        ...
        {(engagementType === EngagementType.AGENT_PROPOSAL || engagementType === EngagementType.OWNER_INVITATION) && counterparty.name && (
            <div className='px-5 py-4 border-b border-border'>
