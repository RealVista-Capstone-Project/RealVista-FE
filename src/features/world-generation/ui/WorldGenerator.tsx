...
import { Upload, X, Box, CheckCircle2, RotateCcw, AlertTriangle, Coins } from 'lucide-react'
  ...
  const renderUploadMode = () => (
    <div className='flex flex-col gap-6'>
      <div className='bg-slate-50 border border-slate-200 rounded-xl p-6'>
        <h3 className='text-lg font-semibold text-slate-900 mb-2'>Tải ảnh quang trắc</h3>
        <p className='text-sm text-slate-500 mb-6'>
          Tải chính xác {REQUIRED_IMAGES_COUNT} ảnh xung quanh trung tâm không gian bất động sản. Cần chụp 8 góc cụ thể để bao phủ 360&deg;.
        </p>
              ...
              <div key={azimuth} className='relative aspect-square rounded-lg border-2 border-dashed border-slate-300 flex flex-col items-center justify-center bg-white overflow-hidden'>
                {capturedImg ? (
                  ...
                  <div className='text-center p-3 text-slate-400'>
                    ...
                    <span className='text-xs font-medium'>Góc</span>
                  </div>
                )}
              </div>
            )
        ...
        {images.length < REQUIRED_IMAGES_COUNT && (
          ...
          >
            ...
            Chọn thêm {REQUIRED_IMAGES_COUNT - images.length} ảnh
          </button>
        )}
      </div>
      ...
      <div className='flex flex-col sm:flex-row items-center gap-4 border-t pt-6'>
        <div className='flex-1 w-full'>
          <label className='block text-sm font-medium text-slate-700 mb-1'>Chất lượng mô hình AI</label>
          <select
            value={selectedModel}
          ...
          >
            <option value='marble-1.0-draft'>Bản nháp — ~45s, 1 credit</option>
            <option value='marble-1.0'>Tiêu chuẩn — ~5m, 3 credit</option>
            <option value='marble-1.1'>Marble 1.1 — ~5m, 3 credit</option>
            <option value='marble-1.1-plus'>Marble 1.1 Plus — ~5m, 5 credit</option>
          </select>
        </div>

        {!quotaLoading && !unlimited && (
          <div className='flex items-center gap-2 px-3 py-1.5 bg-slate-100 rounded-lg text-sm text-slate-600 shrink-0'>
            <Coins className='w-4 h-4' />
            <span className='font-semibold'>{remaining ?? 0} credit</span>
          </div>
        )}

        <button
          onClick={handleGenerate}
          disabled={images.length !== REQUIRED_IMAGES_COUNT || (!unlimited && (remaining ?? 0) < getModelCost(selectedModel))}
    ...
    <div className='flex flex-col items-center justify-center py-16 bg-slate-50 border border-slate-200 rounded-xl px-6'>
      {phase === 'succeeded' ? (
        <div className='text-center'>
          ...
          <h3 className='text-xl font-bold text-slate-900 mb-2'>Tạo thành công</h3>
          <p className='text-sm text-slate-600 mb-6'>Mô hình 3D của bạn đã được tạo thành công.</p>
        ...
        <div className='text-center'>
          ...
          <h3 className='text-xl font-bold text-slate-900 mb-2'>Tạo thất bại</h3>
          <p className='text-sm text-red-600 mb-6'>{error || 'Đã xảy ra lỗi không xác định trong quá trình tạo.'}</p>
        ...
        <div className='w-full max-w-md text-center'>
          ...
          <div className='flex justify-between text-xs font-semibold text-slate-500 mb-8 uppercase tracking-wider'>
            <span className={phase === 'uploading' ? 'text-blue-600' : 'text-green-600'}>Tải lên</span>
            <span className={phase === 'requesting' ? 'text-blue-600' : phase === 'polling' ? 'text-green-600' : ''}>Khởi tạo</span>
            <span className={phase === 'polling' ? 'text-blue-600 animate-pulse' : ''}>Xử lý</span>
          </div>
