...
/**
 * Example: Listing Detail Page with PostHog Tracking
 */
export function ListingDetailExample({ listingId }: { listingId: string }) {
  ...
  }, [listingId, trackListing, viewStartTime]);
  ...
  const handleInquiry = () => {
    trackListing('INQUIRY', listingId, {
      metadata: { type: 'contact_owner' },
    });
  };

  const handleShare = (platform: string) => {
    trackListing('SHARE', listingId, {
      metadata: { platform },
    });
  };
...
/**
 * Example: Search Results with PostHog Tracking
 */
...
/**
 * Example: Homepage Search with PostHog Tracking
 */
...
/**
 * Example: Recommendations Widget (No tracking needed in display)
 */
export function RecommendationsWidget() {
  ...
  const handleListingClick = (listing: RecommendedListing, index: number) => {
    trackListing('CLICK', listing.listing_id, {
      metadata: {
  ...
  return (
    <div className='rounded-lg border p-6'>
      ...
      <div className='space-y-4'>
        {data?.data.recommendations.map((listing, index) => (
          <div
