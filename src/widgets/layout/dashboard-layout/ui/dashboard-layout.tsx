...
const ownerSidebarItems: SidebarMenuItem[] = [
  ...
  { id: 'rental-contracts', label: 'Rental Contracts', href: ROUTES.dashboard.rentalContracts, icon: FileText },
...
export function DashboardLayout({
  ...
  const pageTitle = React.useMemo(() => {
    ...
    if (pathname === ROUTES.dashboard.rentalContracts || pathname.startsWith(ROUTES.dashboard.rentalContracts))
      return t('pageTitle.rentalContracts');
    if (pathname === ROUTES.dashboard.myContracts || pathname.startsWith(ROUTES.dashboard.myContracts))
  ...
  return (
    <div className={cn('flex min-h-screen w-full', className)}>
      ...
      >
        {/* Logo */}
        <div className={cn('flex h-16 items-center border-purple-92/50 p-5', isCollapsed ? 'justify-center' : 'justify-between')}>
          {isCollapsed ? (
            <button
              type='button'
            ...
            >
              <Image src='/logo.png' alt='RealVista Logo' width={32} height={32} className='shrink-0' />
            </button>
          ) : (
            <>
              <Link href={logoHref} className='flex items-center gap-3'>
                ...
                <div className='flex flex-col'>
                  <span className='text-base font-bold leading-tight text-main-black'>Estatery</span>
      ...
      {/* Main Content */}
      <div className={cn('flex flex-1 flex-col transition-all duration-300 ease-in-out', isCollapsed ? 'ml-16' : 'ml-64')}>
        {/* Top Nav */}
